...
const SeatsSelection = () => {
  ...
  const toggleActivatedSeat = useCallback(
    (seat) => dispatch({ type: 'TOGGLE_ACTIVATED_SEAT', seat }),
    [dispatch]
  );
  const updateName = useCallback((name) => dispatch({ type: 'UPDATE_NAME', name }), [dispatch]);
  const updateSurname = useCallback(
    (surname) => dispatch({ type: 'UPDATE_SURNAME', surname }),
    [dispatch]
  );
  const updatePhone = useCallback((phone) => dispatch({ type: 'UPDATE_PHONE', phone }), [dispatch]);
  const updateEmail = useCallback((email) => dispatch({ type: 'UPDATE_EMAIL', email }), [dispatch]);
  const reserveSeats = useCallback(
    (seats) => dispatch({ type: 'RESERVE_SEATS', seats }),
    [dispatch]
  );
  const showSummary = useCallback((e) => dispatch({ type: 'SHOW_SUMMARY', e }), [dispatch]);
  const setReservedSeats = useCallback(
    (data) => dispatch({ type: 'SET_RESERVED_SEATS', data }),
    [dispatch]
  );
  ...
  useEffect(() => {
    axios
      .get(
        `https://karol-cinema.firebaseio.com/reservations/${selectionDay}/${selectionHour}/reservedSeats.json`
      )
      .then((response) => {
  ...
  const bookingHandler = () => {
    const allReservedSeats = [...selectionReservedSeats, ...selectionActiveSeats];
    const customerData = {
    ...
    axios
      .post(
        `https://karol-cinema.firebaseio.com/reservations/${selectionDay}/${selectionHour}/customersData.json`,
        customerData
      )
      .then((response) => {
        console.log(response);
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    seats.push(
      <div className={classes} key={i} onClick={() => toggleActivatedSeat(i)}>
        {i}
      </div>
    );
  }
  ...
  if (selectionActiveSeats.length === 0) {
    summary = <div> Please choose your seats! </div>;
  } else if (selectionBooked) {
    summary = <div> Your seats are sucessfully booked! </div>;
  } else if (
    !selectionBooked &&
    selectionName &&
    selectionSurname &&
    selectionEmail &&
    selectionPhone &&
    selectionActiveSeats.length !== 0
  ) {
    summary = (
      <div>
        <strong>Details of your booking</strong>
        <br />
    ...
    summary = <div> Please fill the form correctly! </div>;
  }
